...
# rmarkdown::render(input = "Region Wise Category Report.Rmd", params = list(report_title = "Category Report xx"), output_file = "Category Report xx 2023.docx")
---
...
region_filter = config$region
# filename = config$filename
xlsx_path = file.path(getwd(), config$region_folder)
...
data = data %>%
  ...
  filter(Region %in% c(region_filter))
...
data.frame(
  Comments = c(
    glue("- The average Classification accuracy YOY for {Accuracy_Region_avg$Region[1]} is {Accuracy_Region_avg$average_percentage[1]}"))
) %>%
  # knitr::kable("pandoc", row.names = FALSE, col.names = NULL) %>%
  flextable() %>%
  # compose(part = "header", j = ~Comments, value = as_paragraph("")) %>%
...
\newpage
## Accuracy based on Sub Vertical `r current_year`
```{r Accuracy_Sub_Vertical, echo = FALSE, ft.align="left"}
data %>%
  filter(Year == current_year) %>%
  pivot_table(.rows = Sub.vertical,
              .columns = Validation.Status,
              .values = ~ round(sum(Spend.Total))) %>%
  mutate(across(-Sub.vertical, ~ scales::dollar_format(scale = .000001, prefix = "$", suffix = "MM")(.))) %>%
  rename_all(stringr::str_to_title) %>%
  mutate(Sub.vertical = stringr::str_to_title(Sub.vertical)) %>%
  flextable() %>%
  theme_booktabs() %>%
  FitFlextableToPage() %>%
  bg(bg = "#A7C6DD", part = 'header') %>%
  ...
  bold(j = "Sub.vertical", bold = TRUE, part = 'all') %>%
  vline(border = officer::fp_border(), part = "body") %>%
  ...
  set_header_labels(values = list(Sub.vertical = "Sub Vertical")) %>%
  border_outer(border = table_border) %>%
  align(align = 'center', part = 'all')
```

## Accuracy based on Category `r current_year`
```{r Accuracy_Category, eval=!any(data$Sub.vertical %in% c("LOGISTICS")),  echo = FALSE, ft.align="left"}
Accuracy_Category = data %>%
  filter(Year == current_year) %>%
  pivot_table(.rows = Category,
              .columns = Validation.Status,
              .values = ~ round(sum(Spend.Total))) %>%
  
  mutate(Category = replace(Category, is.na(Category), "Unknown")) %>%
...
data.frame(
  Comments = c(
    glue("- {Accuracy_Category$Sub.category[1]} has {Accuracy_Category$`To Be Reviewed`[1]} spend which can be reviewed to boost the accuracy further in this space."),
...
Spend_Segmentation_Sub_Vertical_YOY %>%
...
top10_2023 <- Spend_Segmentation_Sub_Vertical_YOY %>% filter(Year == "2023" & vendor_bucket == "PercentTop10") %>% pull(bucket_percent)
top10_2022 <- Spend_Segmentation_Sub_Vertical_YOY %>% filter(Year == "2022" & vendor_bucket == "PercentTop10") %>% pull(bucket_percent)
tail_2023 <- Spend_Segmentation_Sub_Vertical_YOY %>% filter(Year == "2023" & vendor_bucket == "PercentRemainingExcludingTop50") %>% pull(bucket_percent)
tail_2022 <- Spend_Segmentation_Sub_Vertical_YOY %>% filter(Year == "2022" & vendor_bucket == "PercentRemainingExcludingTop50") %>% pull(bucket_percent)
...
## Spend Segmentation by Category `r current_year`
```{r Vendor_Count_Spend_Value_Category, echo = FALSE, ft.align="left"}
required_columns = c("Top10", "Next20", "Next20ExcludingTop30", "RemainingExcludingTop50")

Spend_Segmentation_Category = data %>%
  filter(Year == current_year) %>%
  group_by(Category, ElectrifAi_ParentID) %>%
  summarise(TotalSpend = round(sum(Spend.Total))) %>%
  mutate(Rank = dense_rank(desc(TotalSpend))) %>%
  mutate(Group = case_when(
    Rank <= 10 ~ "Top10",
  ...
  group_by(Category, Group, ElectrifAi_ParentID) %>%
  summarise(GroupSpend = sum(TotalSpend), .groups = 'drop') %>%
  pivot_wider(names_from = Group, values_from = GroupSpend, values_fill = 0) %>%
...
Spend_Segmentation_Category %>%
  ...
  # bg(bg = "#E2E2E2", part = 'body') %>%
  bold(bold = TRUE, part = 'header') %>%
  # bold(j = "Category", bold = TRUE, part = 'body') %>%
  # bg(j = "Category", bg = '#A7C6DD', part = 'all') %>%
  bold(j = "Category", bold = TRUE, part = 'all') %>%
  vline(border = officer::fp_border(), part = "body") %>%
  border_outer(border = table_border) %>%
  set_header_labels(
...
```{r Spend_Segmentation_Category_Plot, echo = FALSE, fig.dim = c(7, 3.5), ft.align="left"}
data %>%
  filter(Year == current_year) %>%
  group_by(Category, ElectrifAi_ParentID) %>%
  summarise(TotalSpend = round(sum(Spend.Total))) %>%
  ...
  pivot_longer(!Category, names_to = "vendor_bucket", values_to = "bucket_percent") %>%
  mutate(Category = stringr::str_to_title(Category),
         vendor_bucket = factor(
    vendor_bucket,
    levels = c("PercentTop10", "PercentNext20", "PercentNext20ExcludingTop30", "PercentRemainingExcludingTop50"))) %>%
  ggplot(aes(x = Category, y = bucket_percent, fill = vendor_bucket)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label=scales::percent(
    bucket_percent,
    suffix = "%",
...
```{r Spend_Segmentation_Cat_Comments, echo = FALSE, ft.align="left"}
# Create the comments
# Create the data frame
data.frame(
  Comments = c(
    glue("- {Spend_Segmentation_Category$Category[1]} has the highest spend of {scales::dollar(Spend_Segmentation_Category$TotalSpend[1], scale = .000001, prefix = '$', suffix = 'MM')} out of which top 10 vendor has {Spend_Segmentation_Category$PercentTop10[1]} spend followed by {Spend_Segmentation_Category$Category[2]} with a spend of {scales::dollar(Spend_Segmentation_Category$TotalSpend[2], scale = .000001, prefix = '$', suffix = 'MM')} out of which top 10 vendor has {Spend_Segmentation_Category$PercentTop10[2]}")
)) %>%
  # knitr::kable("pandoc", row.names = FALSE, col.names = NULL) %>%
  flextable() %>%
  # compose(part = "header", j = ~Comments, value = as_paragraph("")) %>%
